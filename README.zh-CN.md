...
浏览器要选文件时（例如上传图片），列表上会多出 **用 RHFiles 选文件**。点它之后 RHFiles 会打开自己的选择界面：缩略图网格（只列出那个对话框真正接受的文件类型）、实时预览、多选和键盘操作。浏览器原来的对话框全程保持打开，因此确认后会把选中的文件写回去，上传照常完成。

筛选条件是从对话框本身读出来的，不是猜的，所以在任何界面语言下都有效，也会跟随用户手动改过的类型下拉框。取消会焦点还给原对话框；想用浏览器自己的对话框挑也完全不受影响。
