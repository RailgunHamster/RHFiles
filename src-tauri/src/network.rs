...
pub async fn ftp_list(
    ...
    let mut ftp =
        FtpStream::connect((host_only, port)).map_err(|e| format!("FTP connect: {}", e))?;
    ftp.login(&user, &pass)
...
pub async fn sftp_download(
    ...
    let tcp =
        std::net::TcpStream::connect((host.as_str(), port)).map_err(|e| format!("TCP: {}", e))?;
...
pub fn browse_network() -> Result<Vec<FileInfo>, String> {
    ...
    use windows::Win32::NetworkManagement::WNet::{
        ...
        WNetCloseEnum, WNetEnumResourceA, WNetOpenEnumA,
    };
    ...
    unsafe {
        ...
        loop {
            let enum_result = WNetEnumResourceA(
                handle,
                &mut count,
                buf.as_mut_ptr() as *mut _,
                &mut buf_size,
            );
            if enum_result == windows::Win32::Foundation::ERROR_NO_MORE_ITEMS {
                break;
            }
            if enum_result.0 != 0 {
                break;
            ...
            let resources =
                std::slice::from_raw_parts(buf.as_ptr() as *const NETRESOURCEA, count as usize);
